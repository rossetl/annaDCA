...
class ContinuousLabelLayer(Layer):
    def __init__(
        self,
        shape: int | Tuple[int, ...] | torch.Size,
        **kwargs
        ...
        super().__init__(shape=shape, **kwargs)
        assert len(self.shape) == 1, f"ContinuousLabelLayer shape must be one-dimensional, got {self.shape}."
        self.bias = Parameter(torch.zeros(self.shape), requires_grad=False)
    ...
    def init_from_data(
        ...
        Args:
            ...
            pseudo_count (float, optional): Pseudo count to be added to the data frequencies. Defaults to 0.0.
        """
        self.bias.copy_(torch.log(visible.mean(0) / (1 - visible.mean(0)) + 1e-10))
        self.scale.copy_(torch.sqrt(visible.var(0, unbiased=False) + 1e-10))
    ...
    def __repr__(self) -> str:
        return f"ContinuousLabelLayer(shape={self.shape}, device={self.device}, dtype={self.dtype})"
